	...
	CREATE TABLE `pleaser-shoes`.`usuario` (
	  `idusuario` INT NOT NULL AUTO_INCREMENT,
	  `correo` VARCHAR(100) NULL,
	  `password` VARCHAR(30) NULL,
	  `rePassword` VARCHAR(30) NULL,
	  PRIMARY KEY (`idusuario`),
	  UNIQUE INDEX `correo_UNIQUE` (`correo` ASC) VISIBLE);
	  
	  insert into usuario (idusuario, correo, password, rePassword) values
	  (1, 'hector.ht10@gmail.com', '123','123' );
